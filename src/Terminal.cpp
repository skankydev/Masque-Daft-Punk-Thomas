...
void Terminal::setupMap() {
	...
	_commandParam["setSens"]       = &Terminal::setSens;
}
...
void Terminal::help() {
	...
	printLigne("setSens",       "0.1-5.0", "Sensibilité micro (défaut 1.0)",  "vert");
...
void Terminal::setSens(String params) {
	float s = params.toFloat();
	if (s < 0.1f || s > 5.0f) {
		return error("Sensibilité invalide. Plage : 0.1 – 5.0  Ex : setSens 1.5");
	}
	MicManager::getInstance()->setSensitivity(s);
	Serial.println(vert("Sensitivity : ") + String(s, 2));
}
